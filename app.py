...
def all_products():
    ...
    for artwork in artworks:
        ...
        if not artwork.get('sku'):
            print(f"WARNING: Skipping artwork due to missing or invalid SKU: {artwork.get('name', 'Unknown')}")
...
@app.route('/purchase-form', methods=['GET']) # REMOVED 'POST' from methods
@login_required
def purchase_form():
    # This route is now primarily for displaying the form if needed,
    # but the main checkout flow will bypass its POST functionality.
    ...
    user_data_from_db = next((u for u in users if u['id'] == str(user.id)), None)
...
def product_detail(sku):
        artworks = load_artworks_data() # Ensure this helper is correctly defined as well
        artwork = next((a for a in artworks if a.get('sku') == sku), None) # Use .get() for safety
        if not artwork:
            flash('Artwork not found.', 'danger')
            return redirect(url_for('all_products'))
        return render_template('product_detail.html', artwork=artwork)

# Route to process checkout directly from the cart using user's profile details
...
def process_checkout_from_cart():
    ...
    user = current_user
    artworks_data = load_artworks_data()
    cart_summary = calculate_cart_totals(cart, artworks_data)

    if not cart_summary['cart_items']:
        flash('Some items in your cart are out of stock or invalid. Please check your cart.', 'danger')
        return redirect(url_for('cart'))

    customer_name = user.name if user.name else user.email.split('@')[0]
    customer_phone = user.phone if user.phone else 'N/A'
    customer_address = user.address if user.address else 'N/A'
    customer_pincode = user.pincode if user.pincode else 'N/A'
    ...
    new_order = {
        ...
        'invoice_details': {
            ...
            'customer_phone_camouflaged': customer_phone
        }
    }
    ...
    session['cart'] = {}
    flash(f'Order {new_order_id} created successfully! Please review and proceed to payment.', 'success')

    updated_artworks = artworks_data
    for order_item in cart_summary['cart_items']:
